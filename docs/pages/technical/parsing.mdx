...
The Lawtext CST is a sequence of [`Line`](lib/core/modules/core_src_node_cst_line.md#line)s. A `Line` represents a physical line in the Lawtext string and contains all detailed features in the original string (not included in the resulting `StdEL` tree), such as verbose blank lines, meaningless whitespaces, and physical position (line and character index).
